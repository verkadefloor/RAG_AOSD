...
To function, this chatbot requires two backend services running: one for the LLM (`llama.cpp`) and one for TTS (Fish Speech). CUDA is recommended, note the requirement for at least 16GB of available ram. For the development of the app, these services were ran in two Ubuntu-based containers on a proxmox host, with CUDA acceleration. For further details on the models used and their installation instructions, please refer to the linked documents.
